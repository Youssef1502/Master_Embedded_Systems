...
--  Description  :   Write a EXTI Driver for STM32F103X6
==================================================**/
...
typedef struct
{
	uint16_t 		EXTI_InputLineNumber;		// uint16_t because EXTI0 >> EXTI15
	GPIO_TypeDef * 	GPIO_PORT;
	uint16_t 		GPIO_PIN;                   // Must Equal EXTI_InputLineNumber
...
typedef struct
{
	...
	uint8_t IRQ_EN;								// Enable or Disable the EXTI IRQ (That will Enable the IRQ Mask in EXTI and also in the NVIC Controller)
												// This Parameter must be based on @ref EXTI_IRQ_define

	void (*P_IRQ_CallBack) (void);				// Set the C Function which will called once the IRQ Happen
