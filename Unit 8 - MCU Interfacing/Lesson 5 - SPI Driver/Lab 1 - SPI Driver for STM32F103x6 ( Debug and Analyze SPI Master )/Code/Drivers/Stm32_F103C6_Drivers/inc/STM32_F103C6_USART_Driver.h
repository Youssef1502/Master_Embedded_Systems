...
--  Description  :   Write a USART Driver for STM32F103X6
==================================================**/
...
typedef struct
{
	...
	uint8_t USART_Mode;     		// Specifies the TX/RX Enable/Disable Mode.
	...
	uint8_t HwFlowCtl ;				// Specifies whether the hardware flow control mode is enabled or disabled [cts / rts]
									// This parameter must be set based on @ref UART_HwFlowCtl_define


	uint8_t	IRQ_Enable;				// enable or disable UART IRQ TX/RX
									// This parameter must be set based on @ref UART_IRQ_Enable_define , you can select two or three parameters EX.UART_IRQ_Enable_TXE |UART_IRQ_Enable_TC
...
#define UART_Payload_Length_8B               (uint32_t)(0)    // 0 at bit 12
...
#define UART_Parity__NONE                    (uint32_t)(0)    // 0 at bit 9
...
#define UART_StopBits__1_half                (uint32_t)(3<<12)  // = (0b11<<12)
...
void MCAL_UART_SendData	(USART_TypeDef *USARTx, uint16_t *pTxBuffer,enum Polling_mechism PollingEn ); // pTxBuffer is uint16_t because data can be 8 , 9 bits
